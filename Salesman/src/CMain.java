...
public class CMain {
  ...
  static String[] NIAS = {"1397750", "1365445", "1390451", "1359074", "1391791", "1332764",
      "1390203", "1391402", "1395239", "1341199", "1264719", "1307766", "1391540", "1390932",
      "1362857", "1390770", "1341247", "1390934", "1391725", "1390127", "1281175", "1397833",
      "1311996", "1337854", "1395062", "1304626", "1331167", "1390687", "1395359", "1391649",
      "1390450", "1391488", "1391713", "1391106", "1391664", "1358228", "1412759", "1362679",
      "1366706", "1390282", "1391971", "1417251", "1390536", "1272977"};

  static boolean NIACorrecto(String nia) {
    for (int i = 0; i < NIAS.length; ++i)
      if (nia.equals(NIAS[i]))
        return true;
    return false;
  }
  ...
  static void SaveRandomProblem(int n, int nVertices, int nEdges, int nVisits) {
    try {
      System.out.println("SaveRandomProblem " + n + " Vertices=" + nVertices + " Edges=" + nEdges
          + " Visits=" + nVisits);
  ...
  static void SaveCiclicRandomProblem(int n, int nVertices, int nEdges, int nVisits) {
    try {
      System.out.println("SaveCiclicRandomProblem " + n + " Vertices=" + nVertices + " Edges="
          + nEdges + " Visits=" + nVisits);
      CGraph graph = RandomGraph(nVertices, nEdges);
  ...
  private static void testDijkstraTests(CGraph graphTest, int errD2) throws Exception {
    ...
    int errD1;
    {
      for (int i = 0; i < 25; i++) {
        ...
        // print
        System.out
            .println("Dijkstra " + j + " time: " + tempD1 + " Errors: " + errD1 + " DijkstraQueue "
                + j + " time: " + tempD2 + " Errors: " + errD2 + " Time difference: " + tempDiff);
      }
    }
  }
   ...
   * @param numGrafs Number of graphs you want to generate. The number of vertex on each graph
   *        depends on how many graphs you generate. it will do numGraf graphs of i*5 vertex on each
   *        iteration, starting at i=1.
   * @throws Exception
   */
  static void printRandomDijkstraAnalysis() throws Exception {
    ...
    try {
      ...
      for (int i = 1; i <= numGrafs; i++) {
        ...
        System.out.println(
            "Iteration: " + j + " Vertex number: " + numVertex + " Edge number: " + numEdges);
  ...
  static void printAnalysis(String algorihtm) throws Exception {
    ...
    System.out.println("Graphs matching test cases with " + algorihtm + ": " + totalCorrectGraphs
        + ". Total testcase graphs: 25. Total time: " + totalT / 1e9);
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (!NIACorrecto(NIAAlumno1))
      throw new Exception("El NIA " + NIAAlumno1 + " no es de alumno matriculado");
    if (!NIAAlumno2.isEmpty() && !NIACorrecto(NIAAlumno2))
      throw new Exception("El NIA " + NIAAlumno2 + " no es de alumno matriculado");
    ...
    try {
      ...
      if (algorihtm.toLowerCase().equals("dijkstra")
        ...
        if (args.length == 4 && args[3].toLowerCase().equals("salir"))
          salir = true;
