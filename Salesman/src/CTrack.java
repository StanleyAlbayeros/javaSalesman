...
public class CTrack {
  ...
  public CTrack AppendBefore(CTrack t) {
    assert m_Graph == t.m_Graph;
    for (Iterator<CVertex> iter = t.m_Vertices.descendingIterator(); iter.hasNext();)
      m_Vertices.addFirst(iter.next());
    return this;
  }
  ...
  public void Read(String filename) throws Exception {
    ...
    try {
      if (!s.nextLine().equalsIgnoreCase("TRACK"))
        throw new Exception(filename + " no tiene formato de fichero de camino");
      // leer v�rtices
      while (s.hasNextLine()) {
